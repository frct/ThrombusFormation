...
density = np.repeat(np.repeat(density, scale, axis=0), scale, axis=1)
...
for y in y_start:
    ...
    for j,c in enumerate(CFL):
        ...
        cross_indices = np.where(x_mean >= 180 * scale)[0]
    ...
    ax.set_xlim([Nx // 2 - scale * 65,Nx // 2 + scale * 65])
    ax.set_ylim([0,scale * 30])     
